// ===============================
// app.js (clean + GitHub Pages safe)
// ===============================
...
// base-safe URL (works on GitHub Pages subpath)
...
function renderPlaceHTML(p) {
  ...
  const filmsHTML = (p.films || []).map(f => {
    ...
    const note = (f.note && String(f.note).trim())
...
function openPanel(p) {
  if (!overlay || !panel || !panelTitle || !panelContent) return;
...
function renderResults(found) {
  ...
  found.slice(0, 10).forEach(item => {
    ...
    el.innerHTML =
      `${toSafeHTML(item.data.name)}<span class="search-sub">${toSafeHTML(item.data.type)} • ${toSafeHTML(item.data.district)}</span>`;
